...
최종 V2 full-run 종합본 — 이것 하나를 먼저 보면 됩니다:

- [V2 최종 논문형 종합본](reports/full_run_factor_router_synthesis_v2_final_20260530_ko.md)

이 단일 V2 문서는 1.25M date-balanced DDQM2 anchor, 2,082,485-row full-panel long/short matrix, factor-router 자율 연구 리포트까지 하나로 통합한 현재 기준 canonical narrative입니다. 기존 V1 초안도 보존합니다: [영문](reports/full_run_research_paper_20260529_en.md), [한글](reports/full_run_research_paper_20260529_ko.md).
...
Heavy experiment를 실행하지 않고 factor-router surface를 dry-run합니다.
...
4. **Factor-router harness anchor**
   - Sequential harness에 local-only, global-only, global/local quota, family/category cap, factor-count, macro-design, min-weight 축을 추가했습니다.
   - Report의 `category`는 `FactorDefinition.family`의 alias이며, 잘못된 quota/cap 조합은 subprocess 실행 전에 거절합니다.
   - 로컬 artifact만 사용해 gated anchor 한 개를 실행했습니다: baseline mean, q=0.30, selected_13_global_local, N=13, walk-forward OOS.
   - Anchor 결과: 383 OOS periods, cumulative return 12.5557, CAGR 8.51%, MDD -43.07%, turnover 15.24%.
   - Router state는 `candidate`입니다. 더 넓은 결론을 내리기 전 model factor importance, global/local table, leg attribution, worst-drawdown explanation 같은 해석 근거가 더 필요하기 때문입니다.
   - 후속 run은 sequential plan에 적힌 command를 한 번에 하나씩만 실행해야 하며, team/swarm 또는 heavy 병렬 실행은 금지합니다.

## Verification

Factor-router harness completion에서 사용한 최근 검증:
