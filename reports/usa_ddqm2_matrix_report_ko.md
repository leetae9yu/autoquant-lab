...
English version: [`reports/usa_ddqm2_matrix_report_en.md`](usa_ddqm2_matrix_report_en.md)
...
### 2.3 DDQM/DDQM2 원문 결과와 본 실험 결과의 비교

원문 DDQM/DDQM2와 본 프로젝트의 결과는 직접적인 수익률 숫자 비교로 읽으면 안 된다. 원문은 한국시장/KOSPI200 계열 universe와 원문 데이터, 원문 factor set, 원문 거래/평가 조건을 사용하고, 본 프로젝트는 U.S. equity monthly panel, CRSP/Compustat/IBES/FRED-style 로컬 artifact, date-balanced prepared panel, U.S. macro proxy, selected 13-factor universe를 사용한다. 따라서 이 절의 목적은 “누가 더 높은 수익률을 냈는가”가 아니라, 원문에서 관찰된 방법론적 효과가 U.S. adaptation에서도 같은 방향으로 나타나는지 보는 것이다.

| 항목 | DDQM | DDQM2 | autoquant-lab U.S. adaptation |
|---|---|---|---|
| 시장/Universe | 한국시장/KOSPI200 계열 | 한국시장/KOSPI200 계열 | U.S. equity monthly panel, 1.25M-row date-balanced prepared panel |
| 핵심 예측 대상 | factor regime/style 국면 | factor별 다음 기간 long-short return | factor별 다음 1개월 long-short return |
| 주요 모델 | Random Forest 계열 classification | LightGBM 계열 regression | LightGBM 중심, ridge/elasticnet/tree ensemble/baseline 비교 |
| portfolio construction | 예측 국면에 따른 factor rotation | 예측 factor return 기반 dynamic allocation | weighted factor-return surface와 stock-score QSpread surface 둘 다 평가 |
...
2. **동적 factor allocation이 핵심이다.** 단순히 좋은 factor를 하나 고정하는 방식이 아니라, 매월 macro/market state에 따라 factor weight를 바꾸는 구조가 DDQM2의 핵심과 맞았다.
3. **최종 portfolio surface가 중요했다.** 본 실험에서는 weighted factor-return surface보다 stock-level weighted factor score QSpread surface가 DDQM2의 최종 포트폴리오 구성에 더 가까웠고, q=0.20/q=0.30 후보에서 더 강한 결과를 보였다.

다만 원문 대비 아직 부족한 부분도 명확하다.

- 원문과 동일한 한국시장 universe가 아니므로 replication이 아니라 adaptation이다.
- 원문 factor 13개를 1:1로 복제하지 않았고, EQR registry에서 selected 13-factor universe를 구성했다.
- transaction cost, slippage, borrow cost, tax, capacity가 아직 net return에 반영되지 않았다.
- 원문 결과와 같은 표준으로 비교하려면 동일한 평가 기간, 동일한 거래비용 가정, 동일한 universe 정의가 필요하다.

### 2.4 AI agent 관점에서 새로 바꾼 부분
